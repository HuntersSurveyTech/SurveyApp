...
namespace Microsoft.WindowsAzure.Storage.Table.Queryable
{
    ...
    internal class ExpressionNormalizer : DataServiceALinqExpressionVisitor
    {
        ...
        internal override Expression VisitBinary(BinaryExpression b)
        {
            ...
            Pattern pattern;
            if (this.patterns.TryGetValue(visited.Left, out pattern) && pattern.Kind == PatternKind.Compare && IsConstantZero(visited.Right))
            {
        ...
        internal Expression VisitMethodCallNoRewrite(MethodCallExpression call)
        {
            ...
            {
                return this.CreateCompareExpression(visited.Arguments[0], visited.Arguments[1]);
            }

            if (!visited.Method.IsStatic && visited.Method.Name == "CompareTo" && visited.Arguments.Count == 1 && visited.Method.ReturnType == typeof(int))
            {
                return this.CreateCompareExpression(visited.Object, visited.Arguments[0]);
            }
        ...
        private Expression CreateCompareExpression(Expression left, Expression right)
        {
            ...
            this.patterns[result] = new ComparePattern(left, right);
